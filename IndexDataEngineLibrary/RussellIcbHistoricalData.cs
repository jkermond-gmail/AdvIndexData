...
{
    public sealed class RussellIcbHistData
    {
        ...
        public void ProcessVendorFiles2(DateTime oStartDate, DateTime oEndDate, string Dataset, bool bOpenFiles, bool bCloseFiles,
                                       bool bTotalReturnFiles, bool bSecurityMaster, bool bSymbolChanges)
        {
            DateTime oProcessDate;
            int DateCompare;
            string FilePath = AppSettings.Get<string>("VifsPath.RussellICB_history");
                   ...
                   ; oProcessDate = oProcessDate.AddDays(1))
                {
                    if(bCloseFiles)
                    {
                        ...
                        if(File.Exists(FileName))
                        {
                            ...
                            if(bCloseFiles)
                            {
                                AddRussellClosingData2(VendorFileFormats.H_CLOSE_ICB, FileName, oProcessDate);
                            }
        ...
        private void AddRussellClosingData2(VendorFileFormats FileFormat, string FileName, DateTime FileDate)
        {
            SqlConnection cnSql = new SqlConnection(sharedData.ConnectionStringIndexData);
            cnSql.Open();
            string sTable = "RussellDailyHoldings1";
            string SqlUpdate = "update " + sTable + " set SectorICB = @SectorICB ";
            string SqlWhere = "where FileDate = @FileDate and CUSIP = @CUSIP ";
            SqlCommand cmdHoldingsRec = new SqlCommand(SqlUpdate + SqlWhere, cnSql);
            cmdHoldingsRec.Parameters.Add("@FileDate", SqlDbType.DateTime, 8);
            cmdHoldingsRec.Parameters.Add("@CUSIP", SqlDbType.VarChar, 8);
            cmdHoldingsRec.Parameters.Add("@SectorICB", SqlDbType.VarChar, 8);
                ...
                if(ok)
                {
                    ...
                    try
                    {
                        if(ok)
                        {
                            cmdHoldingsRec.Parameters["@CUSIP"].Value = sCUSIP;
                            cmdHoldingsRec.Parameters["@FileDate"].Value = oDate;
                            cmdHoldingsRec.Parameters["@SectorICB"].Value = SectorICB;
                            cmdHoldingsRec.ExecuteNonQuery();
                        }
                    }
        ...
        public void GenerateIndustryReturnsForDate(string sDate, string sIndexName)
        {
            ...
            if(GenerateReturnsForDate2(sDate, sIndexName, AdventOutputType.Sector) == true)
            {
        ...
        public void GenerateConstituentReturnsForDate(string sDate, string sIndexName)
        {
            ...
            if(GenerateReturnsForDate2(sDate, sIndexName, AdventOutputType.Constituent) == true)
            {
                for(bool GotNext = true; GotNext;)
                {
                    i += 1;
        ...
        private bool GenerateReturnsForDate2(string sDate, string sIndexName, AdventOutputType OutputType)
        {
            string sMsg = null;
            mSqlConn = new SqlConnection(sharedData.ConnectionStringIndexData);
            bool ReturnsGenerated = false;
            ...
            try
            {
                sMsg = "GenerateReturnsForDate2: " + sDate + " Index: " + sIndexName;
                LogHelper.WriteLine(sMsg);

                string SqlSelect = @"
                    SELECT h1.FileDate, h2.IndexName, h1.CUSIP, lower(h1.Ticker) as Ticker, h1.SecurityReturn, 
                    LEFT(h1.SectorICB,2) As IcbIndustry, LEFT(h1.SectorICB,4) As IcbSuperSector, LEFT(h1.SectorICB,6) As IcbSector, h1.SectorICB As IcbSubSector,
                        ROUND((( (cast(h1.MktValue as float) * 
                                    (cast(h2.SharesNumerator as float)/h1.SharesDenominator))/
                        (SELECT     
                            sum( cast(h1.MktValue as float) * 
